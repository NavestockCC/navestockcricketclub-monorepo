...
export class PublishPubSubMessage {
  ...
  public publishPubSubMessage(pubsubTopicName: string, messagedata: any) {
    ...
    if (typeof messagedata === 'string' || messagedata instanceof String) {
      ...
      typeof messagedata === 'boolean'
    ) {
      dataBuffer = Buffer.from(messagedata.toString());
    }

    const publishTopic = from(pubsubTopic.publishMessage({ data: dataBuffer }));
    return publishTopic.pipe(
      map((r) => r),
      retry(2),
      catchError((error) => {
        functions.logger.debug(`pubsubTopic: ${error}`);
        if (error.code == 5) {
          const managedError = from(
            pubSubClient.createTopic(pubsubTopicName)
          ).pipe(
            switchMap(() =>
              from(pubsubTopic.publishMessage({ data: dataBuffer }))
            ),
            catchError((err) => {
              throw (
                'Second Attempt to publish PubSub Message Error Details: ' + err
              );
            })
          );
          return managedError;
