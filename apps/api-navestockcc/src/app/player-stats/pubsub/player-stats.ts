...
export const getPlayerStatsPubSub = functions
    ...
    const playerstatsSub = {
      next: (psd) => {
       ...
       //Firestire Document Path to which PlayerStats are written 
        const playerStatPath = afs
        .collection('PlayerStats')
        .doc(psd.player_id);

        const playerStatsDocPath = 
          playerStatPath
          .collection('matches')
          .doc(psd.matchdescription.id.toString());

      //Add Doc Writes to Firestore batch write    
         fsBatch.set(playerStatPath, {'player_id': psd.player_id, 'player_name': psd.player_name});
  ...
  combineLatest([matchDescription, plyrStats]).pipe(
  map(v => {
    let pso = {};
