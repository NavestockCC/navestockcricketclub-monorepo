...
export const comparePlayCricketMatchListPubSub = functions
  ...
  .pubsub.topic('PlayCricket_Match_List_Data')
  .onPublish(async (msgPayload) => {
     ...
     */
    const matchesToUpdate = mlDB.getMatchListImportData(seasonToImport).pipe(
      mergeMap((mlData) =>
      ...
      mergeMap((matchToImport) =>
        pubSubWrite.publishPubSubMessage('Match_Detail_Import', matchToImport)
      )
    );
     ...
     */
    const matchListImportDB = mlDB.setMatchListImportData(
      payloadData,
      seasonToImport
    );
     ...
     */
    const compareMatchList = concat(matchesToUpdate, matchListImportDB);
     ...
     */
    return await lastValueFrom(compareMatchList).catch((e) =>
      functions.logger.debug(`comparePlayCricketMatchListPubSub: ${e}`)
    );
  });
